...
type Props = {
  ...
  variant: "default" | "offshore";
};

export default function OffshoreCompaniesUae({
  data,
  variant = "default",
}: Props) {
  return (
      ...
      <div className="container">
        <div className="typography mb-[25px] sm:mb-[20px] xl:mb-[25px] 2xl:mb-[30px] 3xl:mb-[40px]">
          <Heading
            ...
            size="h2"
            className={cn(
              "text-[#1C5396] mb-[20px] xl:mb-[30px] 3xl:mb-[40px]",
              variant === "offshore" && "xl:mb-[30px]",
            )}
          >
            {data?.title}
          </Heading>
          <div className="text-[14px] xl:text-[18px] 2xl:text-[18px] 3xl:text-[22px] leading-normal font-normal text-[#364153]">
            {data?.description}
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[30px] sm:gap-[20px] xl:gap-[30px] 2xl:gap-[35px] 3xl:gap-[40px] mb-[30px] sm:mb-[20px] xl:mb-[25px] 2xl:mb-[30px] 3xl:mb-[40px]">
          {data?.free_zone_list?.map((item, index) => (
            <div key={item?.id} className="w-full h-auto">
              <div
                className={cn(
                  "w-full h-full p-[30px_15px] sm:p-[35px_25px] xl:p-[55px_35px] 2xl:p-[50px_35px] 3xl:p-[65px_45px] bg-gradient-to-br from-white border-2 rounded-[15px] sm:rounded-[10px] xl:rounded-[15px] 2xl:rounded-[20px] overflow-hidden gap-[20px] sm:gap-[15px] xl:gap-[15px] 2xl:gap-[25px] 3xl:gap-[35px] flex flex-col justify-between",
                  index % 2 !== 0
                    ? "to-[#FEFCE8] border-[#FFCB18]"
                    : "to-[#EFF6FF] border-[#5280CA]",
                )}
              >
                <div className="text-[24px] sm:text-[20px] xl:text-[30px] 2xl:text-[32px] 3xl:text-[40px] leading-normal font-medium text-[#053269]">
                  {parse(item?.title)}
                </div>
                <div className="gap-[20px] xl:gap-[30px] flex flex-col">
                  ...
                  <div>
                    <div className="text-[14px] xl:text-[16px] 2xl:text-[18px] 3xl:text-[21px] leading-normal font-medium text-[#053269] mb-[10px] xl:mb-[15px] 2xl:mb-[20px]">
                      Key Points:
                    </div>
                    <ul className="space-y-[10px] xl:space-y-[15px] 3xl:space-y-[25px]">
                      {item?.zone_list?.map((list, listIndex) => (
                        <li
                          key={listIndex}
                          className={cn(
                            "w-full h-auto pl-[30px] sm:pl-[25px] xl:pl-[30px] 2xl:pl-[35px] 3xl:pl-[40px] relative z-0 before:content-[''] before:w-[20px] sm:before:w-[18px] xl:before:w-[20px] 2xl:before:w-[20px] 3xl:before:w-[25px] before:aspect-square before:bg-no-repeat before:bg-contain before:absolute before:left-0 before:top-[2px]",
                            index % 2 !== 0
                        ...
                        >
                          <span className="text-[14px] xl:text-[16px] 2xl:text-[18px] 3xl:text-[22px] text-[#364153]">
                            {parse(list)}
                          </span>
                        </li>
                      ))}
