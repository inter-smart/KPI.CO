  ...
  return (
    <section className="w-full h-auto py-[20px] sm:py-[40px_50px] xl:py-[50px_60px] 2xl:py-[50px_60px] 3xl:py-[60px_80px] block">
      <div className="container">
        <div
          className={cn(
            "w-full h-auto p-[30px_15px] sm:p-[35px_25px] xl:p-[50px_45px_35px] 2xl:p-[50px_40px] 3xl:p-[70px_50px] border-2 rounded-[15px] 2xl:rounded-[20px] overflow-hidden block",
            variant === "blue"
            ...
            {data?.tax_list?.map((item, index) => (
              ...
              >
                <div className="w-full h-full">
                  ...
                  {item?.description && (
                    <div className="text-[14px] xl:text-[16px] 2xl:text-[18px] 3xl:text-[20px] leading-relaxed font-normal text-[#4E4E4E]">
                      {parse(item?.description)}
                    </div>
                  )}
                  {item?.items && (
                    <ul className="space-y-[10px] xl:space-y-[10px] 3xl:space-y-[25px]">
                      {item?.items.map((item, index) => (
                        <li
                          key={index}
                          className={cn(
                            "w-full h-auto pl-[30px] sm:pl-[25px] xl:pl-[27px] 2xl:pl-[35px] 3xl:pl-[40px] relative z-0 before:content-[''] before:w-[20px] sm:before:w-[18px] xl:before:w-[20px] 3xl:before:w-[25px] before:aspect-square before:bg-no-repeat before:bg-contain before:absolute before:left-0 before:top-[2px] ",
                            variant === "blue"
                        ...
                        >
                          <span className="text-[14px] xl:text-[16px] 2xl:text-[18px] 3xl:text-[20px] leading-normal font-normal text-[#4E4E4E] tracking-[-0.31px]">
                            {parse(item)}
                          </span>
                        </li>
                      ))}
