...
export default function BusinessLocation({
  ...
  return (
    <section
      className={cn(
        "w-full py-[35px] xl:py-[50px] 2xl:py-[70px] 3xl:py-[110px] bg-white",
        variant === "Formation-ADGM" && "xl:pb-[60px]",
      )}
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 xl:gap-[28px] 2xl:gap-8">
          {data.items.map((item) => (
            <div
              key={item.id}
              className={cn(
                "group relative min-h-[234px] xl:min-h-[230px] 2xl:min-h-[260px] 3xl:min-h-[310px] rounded-[10px] shadow-[0px_4px_6px_rgba(0,0,0,0.1)] overflow-hidden shadow-md",
                variant === "Formation-ADGM" && "xl:min-h-[230px]",
                variant === "RAK-Offshore" && "xl:min-h-[230px]",
              )}
            >
              ...
              <div className="relative h-full w-full group-hover:opacity-0 aspect-[380/234]">
                ...
                <div
                  className={cn(
                    "absolute bottom-0 left-0 p-[32px_24px] md:p-[40px_30px]",
