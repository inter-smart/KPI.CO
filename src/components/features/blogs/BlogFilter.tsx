...
import { FilterItem } from "./BlogList";
...
export default function BlogFilter({ categories, activeFilters, onFilterChange, onApply, onClear, selectedSort }: BlogFilterProps) {
  ...
  const toggleFilter = (category: FilterItem) => {
    if (tempFilters.includes(category)) {
      setTempFilters(tempFilters.filter((f) => f.id !== category.id));
    } else {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-[95%] lg:max-w-[900px] p-[25px] xl:p-[32px] 2xl:p-[40px] border-none shadow-2xl rounded-[10px] overflow-visible">
        <DialogTitle className="sr-only">Blog Filters</DialogTitle>
        ...
        <div data-lenis-prevent="true" className="space-y-10 max-h-[65vh] xl:max-h-[70vh] overflow-y-auto overscroll-contain touch-pan-y pr-2 md:pr-4 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-gray-400">
          {/* Sort Selection */}
          ...
          {categories.map((category) => (
            <div key={category.name} className="mb-[30px]">
              <h3 className="text-[14px] sm:text-[18px] 2xl:text-[20px] 3xl:text-[24px] font-semibold text-[#1C5396] tracking-tight mb-[15px]">
                {category.name}
              </h3>

              <div className="flex flex-wrap gap-4">
                {category.children.map((child) => {
                  const isActive = tempFilters.some((f) => f.id === child.id);
                  return (
                    <button
                      key={child?.id}
                      ...
                      className={cn(
                        ...
                        isActive ? "bg-[#f0f7ff] border-none flex items-center gap-3" : "bg-white border-black-40 hover:border-gray-300",
                      )}
                    >
                      {child?.name}
