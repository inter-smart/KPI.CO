    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
                <button className="text-[16px] md:text-[12px] xl:text-[16px] 2xl:text-[17px] 3xl:text-[21px] flex items-center gap-2.5 px-6 py-2.5 rounded-[13px] text-[#4E4E4E] font-medium shadow-[0_0_26px_rgba(0,0,0,0.05)] focus:outline-none">
            ...
            <DialogContent className="max-w-[700px] p-10 border-none shadow-2xl rounded-[20px] overflow-visible">
                {/* Custom Close Button matching image */}
                <DialogClose className="absolute right-6 top-6 w-8 h-8 flex items-center justify-center bg-[#FFC107] rounded-full text-black hover:bg-[#FFB300] transition-colors focus:outline-none z-50">
                ...
                <div className="space-y-10">
                    ...
                    <div className="relative inline-block">
                        <button
                            ...
                            className={cn(
                                "flex items-center gap-3 px-5 py-2.5 bg-white border rounded-xl text-[15px] text-gray-600 font-medium shadow-sm transition-all min-w-[200px] justify-between",
                                isSortOpen ? "border-gray-300 ring-2 ring-gray-100 rounded-b-none" : "border-gray-200"
                            )}
                        >
                            <span>
                                Sort by <span className="text-[#1c5396] font-bold ml-1">{selectedSort}</span>
                            </span>
                            <ChevronDown
                                size={18}
                        ...
                        {/* Dropdown Menu */}
                        {isSortOpen && (
                            <div className="absolute left-0 top-[100%] w-full bg-[#f2f2f2] border-x border-b border-gray-100 rounded-b-2xl overflow-hidden z-40 shadow-xl">
                                {sortOptions.map((option, idx) => (
                                    <button
                                        ...
                                        className={cn(
                                            "w-full px-5 py-3 text-[14px] font-bold text-left transition-colors",
                    ...
                    <div className="space-y-6">
                        <h3 className="text-[28px] font-bold text-[#1c5396] tracking-tight">Categories</h3>
