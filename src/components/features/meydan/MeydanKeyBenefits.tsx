...
export default function MeydanKeyBenefits({ data }: MeydanKeyBenefitsProps) {
  ...
  return (
    <section className="w-full h-auto py-[65px_90px] block">
      <div className="container">
        <div className="typography">
          <Heading
            as="h2"
            size="h2"
            className="text-[#1C5396] mb-[20px] lg:mb-[25px] 2xl:mb-[30px] 3xl:mb-[55px]"
          >
            {data?.title}
          </Heading>
        </div>
        <div className="w-full h-auto py-[85px] bg-[#f5fbfe] rounded-[15px] overflow-hidden block">
          <div className="w-full h-auto pl-[40px] mb-[50px]">
            <Swiper
              slidesPerView="auto"
              spaceBetween={55}
              freeMode={true}
            ...
            >
              {data?.slides?.map((item, index) => (
                <SwiperSlide key={item?.id} className="!w-auto !h-auto">
                  <button
                    className={`text-[30px] leading-normal font-normal text-[#4E4E4E] pb-[5px] whitespace-nowrap cursor-pointer relative z-0 before:content-[''] before:w-full before:h-[3px] before:bg-[#FFC916] before:opacity-0 before:absolute before:z-1 before:inset-[auto_0_0_0] ${
                      activeIndex === index &&
            ...
            >
              {data?.slides.map((item) => (
                <SwiperSlide key={item.id}>
                  <div className="gap-10 lg:gap-[35px] flex flex-col lg:flex-row items-center">
                    ...
                    <div className="w-full lg:w-1/2">
                      <div className="w-full h-full rounded-[15px] overflow-hidden block">
                        <Image
                          src={item?.media?.path}
                          alt={item?.media?.alt}
                          width={770}
